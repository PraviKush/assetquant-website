...
// V23 navigation safety: dedicated contact page is the canonical enquiry route.
document.querySelectorAll('a[href="./contact.html"],a[href="contact.html"]').forEach(function(a){
  a.addEventListener('click', function(e){
    // Normal browser navigation; this explicit handler prevents legacy modal handlers
    // from hijacking the click in older builds.
...
(function(){
 ...
 if(!panel)return;
 const flow=panel.querySelector('.panel-flow');
 if(flow){
   flow.classList.add('aq26-flow');
   flow.insertAdjacentHTML('beforeend','<div class="aq26-pulse" aria-hidden="true"></div>');
 }
 const deco=panel.querySelector('.panel-decoration');
 if(deco){
   deco.classList.add('aq26-signal');
   deco.innerHTML='<div class="aq26-radar"><i></i><i></i><i></i><b>LIVE</b></div><div class="aq26-exception"><span>EXCEPTION DETECTED</span><strong>Signal → Evidence → Action</strong><small>Human-reviewed intelligence</small></div>';
 }
 // animated active state across the 3 capability steps
 const steps=()=>Array.from(panel.querySelectorAll('.panel-flow span'));
 let idx=0;
 setInterval(()=>{
   const a=steps(); if(!a.length)return;
   a.forEach((x,i)=>x.classList.toggle('aq26-active',i===idx%a.length));
   idx++;
 },1800);
})();
