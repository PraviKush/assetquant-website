...
if (context) { let width = 0, height = 0, frame = 0, visible = true, pointer = { x: 0, y: 0 }, raf = null; const nodes = Array.from({ length: 110 }, (_, index) => { const angle = index * 2.399963229728653, z = 1 - (index + .5) * 2 / 110, radius = Math.sqrt(1 - z * z); return { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius, z }; });
